...
public class UserInfoForm extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        userInfoRef = database.getReference(path).child("message");

        userInfoRef.setValue("Hello, World!");
    }

    public void done(View v){
        final String name,home,workAddress,day,month,year,dob;
        ...
        if(TextUtils.isEmpty(name) || TextUtils.isEmpty(home) || TextUtils.isEmpty(day)|| TextUtils.isEmpty(month)||TextUtils.isEmpty(year)){
            ...
            Toast.makeText(UserInfoForm.this, "Please fill the form properly", Toast.LENGTH_SHORT).show();
            return;
        }
        dob=day+"-"+month+"-"+year;
        //Since Workaddress is an optional field
        if(TextUtils.isEmpty(workAddress)){
            userInfoData=new UserInfoData(name,dob,home);
        }
        else
            userInfoData=new UserInfoData(name,dob,workAddress,home);
